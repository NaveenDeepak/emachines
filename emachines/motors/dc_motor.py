...
def calculate_magnetic_circuit(
    ...
    # Air gap
    pole_face_area = (
        geom.magnet_arc_angle_deg / 360
        * np.pi * geom.rotor_outer_diameter * geom.stack_length
    )
    slot_pitch = np.pi * geom.rotor_outer_diameter / geom.num_armature_slots
    k_c = calculate_carter_coefficient(
        geom.air_gap_length, geom.slot_opening_width, slot_pitch
    )
    ...
    R_stator = (
        np.pi * (geom.stator_outer_diameter - yoke_t) / geom.num_poles
    ...
    R_rotor = (
        np.pi * (geom.rotor_inner_diameter + core_t) / geom.num_poles
...
def plot_efficiency_map(
    ...
    ax.plot(
        steady_state_data["torque"], steady_state_data["speed_rpm"],
    ...
    if target_torque is not None and target_speed_rpm is not None:
        ...
        ax.plot(
            target_torque, target_speed_rpm, "*",
